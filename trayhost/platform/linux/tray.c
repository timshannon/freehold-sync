...
static GtkWidget *get_menu()
{
    ...
    openMenuItem = gtk_menu_item_new_with_label("Open Freehold-Sync in Browser");
...
static void set_app_indicator(const char* title, GtkWidget *indicator_menu) {
	/* Indicator */
...
void native_loop(const char* title, unsigned char *imageData, unsigned int imageDataLen) 
{
	...
	set_app_indicator(title, get_menu());
